...
const Header = () => {
    ...
    return (
        <header className="bg-black shadow-sm sticky top-0 z-50 border-b border-gray-800">
            {/* Bulletin Bar */}
            <div className="bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 text-black py-1 px-2 sm:px-4">
                <div className="luxury-container">
                    <div className="flex justify-between items-center">
                        {/* Auto-changing content */}
                        <div className="flex-1 text-center">
                            <p className="text-xs sm:text-sm font-light italic tracking-wide animate-pulse text-black">
            ...
            {/* Main Navigation */}
            <div className="luxury-container px-2 sm:px-4 lg:px-6">
                <div className="flex justify-center items-center h-16 sm:h-20 lg:h-24 gap-4 sm:gap-6 lg:gap-8 xl:gap-12 pt-1 sm:pt-2">
                    {/* Left Navigation - Closer to logo */}
                    <nav className="hidden xl:flex items-center space-x-2 2xl:space-x-3">
                        {leftNavigationItems.map((item) => (
                            <Link
                                key={item.label}
                                href={item.href}
                                className="text-white hover:text-[#d4af37] text-xs xl:text-sm font-medium tracking-wider transition-colors duration-300 uppercase relative group px-1 xl:px-2"
                            >
                                {item.label}
                    ...
                    <div className="flex justify-center">
                        <Link href="/" className="flex items-center group">
                            <Image
                                ...
                                width={180}
                                height={40}
                                className="transition-opacity duration-300 group-hover:opacity-80 w-32 h-auto sm:w-40 md:w-48 lg:w-52 xl:w-56"
                                priority
                            />
                        </Link>
                    </div>

                    {/* Right Navigation - Closer to logo */}
                    <nav className="hidden xl:flex items-center space-x-2 2xl:space-x-3">
                        ...
                        <Link
                            href="/appointment"
                            className="bg-[#d4af37] hover:bg-[#b8941f] text-black px-3 xl:px-4 py-2 text-xs xl:text-sm font-semibold tracking-wider transition-all duration-300 uppercase transform hover:scale-105 shadow-md hover:shadow-lg ml-2 xl:ml-3"
                        >
                            BOOK NOW
                        </Link>
                    </nav>

                    {/* Mobile menu button - Absolute positioning for mobile */}
                    <div className="xl:hidden absolute right-2 sm:right-4">
                        <button
                            onClick={() => setIsMenuOpen(!isMenuOpen)}
                            className="text-white hover:text-[#d4af37] focus:outline-none transition-colors duration-300 p-2"
                ...
                {isMenuOpen && (
                    <div className="xl:hidden">
                        <div className="px-2 sm:px-4 pt-2 pb-4 space-y-2 bg-black border-t border-gray-700 shadow-xl">
                            {/* Navigation Items */}
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                {[...leftNavigationItems, ...rightNavigationItems].map((item) => (
                                    <Link
                                        key={item.label}
                                        href={item.href}
                                        className="block px-3 sm:px-4 py-3 text-white hover:text-[#d4af37] hover:bg-gray-800 text-sm font-medium tracking-wider uppercase transition-colors duration-300 rounded-md border border-gray-700 hover:border-[#d4af37]"
                            ...
                            <Link
                                href="/appointment"
                                className="block px-4 py-3 bg-[#d4af37] hover:bg-[#b8941f] text-black text-sm font-semibold text-center tracking-wider uppercase transition-colors duration-300 rounded-md shadow-lg mt-4"
                                onClick={() => setIsMenuOpen(false)}
                            >
                                BOOK NOW
                            </Link>
